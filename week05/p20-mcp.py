...
def call_llm(messages, tools=None):
    """最小可运行的 LLM mock，用于演示流程。"""
    last_message = messages[-1]["content"]
    if tools:
        if "多少钱" in last_message or "运费" in last_message:
            return {
                "tool_calls": [
                    {
                        "id": "tool_call_1",
                        "function": {
                            ...
                            "arguments": json.dumps(
                                {"weight": 5, "distance": 1200, "company": "顺丰"},
                                ensure_ascii=False
                            ),
                        },
                    }
    ...
    if messages[-1]["role"] == "tool":
        return f"好的，结果如下：{messages[-1]['content']}"

    if "<tool_use>" in messages[0]["content"]:
        return """
<tool_use>
...
def demo_function_calling():
    ...
    print("Function Calling 最终回复:", final_response)
...
class Client:
    ...
    async def list_tools(self):
        return [
            {"name": "query_logistics_companies", "description": "查询支持的物流公司信息"},
            {"name": "calculate_shipping_fee", "description": "计算运费"},
            {"name": "estimate_delivery_time", "description": "估算配送时效"},
            {"name": "track_shipment", "description": "查询物流状态"},
        ]

    async def call_tool(self, name, arguments):
        if name == "calculate_shipping_fee":
            return calculate_shipping_fee(
                arguments["weight"], arguments["distance"], arguments["company"]
            )
        if name == "estimate_delivery_time":
            return estimate_delivery_time(
                arguments["from_city"], arguments["to_city"], arguments["company"]
            )
        if name == "query_logistics_companies":
            return query_logistics_companies()
        if name == "track_shipment":
            return track_shipment(arguments["tracking_number"])
        return "未知工具调用"
...
"""
    ...
    messages.append({
        "role": "tool",
        "content": result,
        "tool_call_id": tool_use.get("id") if tool_use else "tool_call_1"
    })
    final_response = call_llm(messages)
    print("MCP 最终回复:", final_response)
