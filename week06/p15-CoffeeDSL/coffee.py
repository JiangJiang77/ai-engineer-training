...
class CoffeeDSLWorkflow:
    ...
    def intent_recognition_node(self, state: State) -> Dict[str, Any]:
        ...
        # if any(keyword in user_input for keyword in modify_keywords) and any(
        #     keyword in user_input for keyword in time_keywords
        # ):
        #     intent = "modify_rule"
        # else:
        #     intent = "unknown"
        ...
        prompt = f"""
            你是意图识别专家。请分析用户输入，并从以下分类中选择最合适的
            1. modify_rule: 用户想要修改咖啡机工作流程或者参数（如时间、温度、设置等）
            2. unknown: 用户想要做其他事情，或者是与咖啡机设置无关的内容

            用户输入：“{user_input}”
            注意：请只返回意图的ID（modify_rule 或 unknown），不要添加任何解释或格式化标记。
            """

        # 调用通义千问 API
        response = Generation.call(
            model="qwen-turbo", prompt=prompt, max_tokens=1000, temperature=0.1
        )

        intent = response.output.text.strip()

        print("intent_recognition_node intent:")
        print(intent)
        ...
        return {**state, "intent": intent, "current_dsl": current_dsl}
    ...
    def dsl_update_node(self, state: State) -> Dict[str, Any]:
        ...
        # 调用通义千问 API
        response = Generation.call(
            model="qwen-turbo", prompt=prompt, max_tokens=2000, temperature=0.1
        )

        print("dsl_update_node output:")
        ...
        return {**state, "updated_dsl": updated_dsl}
    ...
    def syntax_validation_node(self, state: State) -> Dict[str, Any]:
        ...
        return {**state, "validation_result": True}
